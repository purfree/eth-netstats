...
var ewsConfig = {
  username: process.env.EMAIL_UNAME,
  password: process.env.EMAIL_PWORD,
...
var Collection = function Collection(externalAPI)
{
	...
	//初始化ews
	this._ews = new EWS(ewsConfig);

	if (process.env.WHITELIST){
		this._whitelist = process.env.WHITELIST.split(',');

	}

	if (process.env.EMAIL_RECIPIENTS){
		var recipients = process.env.EMAIL_RECIPIENTS.split(',');
		this._recipients = [];
		for (var i=0;i < recipients.length;i++){
			this._recipients[i] = {"EmailAddress" : recipients[i]}
		}
	}
...
Collection.prototype.sendmail_block = function(block){
	if (this._whitelist){
		for (var i = 0; i < this._whitelist.length; i++) {
			if (block.miner == this._whitelist[i]){
				return
			}
		}
	}
	this.sendmail("块号：" + block.number + "，矿工：" + block.miner);
}

Collection.prototype.sendmail = function(value){
	...
	this._ews.run(ewsFunction, ewsArgs)
	  .then(result => {
	  	console.success("sendmail success ====>", JSON.stringify(result));
	  })
	  .catch(err => {
	  	console.error("sendmail fail ====>", err.stack);
	  });
}
	...
	else
	{
		...
		else
		{
			...
			if(block.block.number > this._highestBlock)
			{
				...
				this.sendmail_block(block.block)
				// console.success("====>>", id, block.block.number)
			}
